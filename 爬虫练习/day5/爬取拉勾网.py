...
def insert_date(job_name,work_place,salary,job_require,company_name,work_keyword,position_id):
    try:
        cur = conn.cursor()
        result = cur.execute('INSERT INTO pythonshanghai(job_name,work_place,salary,job_require,company_name,work_keyword,position_id) VALUES(%s,%s,%s,%s,%s,%s,%s)',(job_name, work_place, salary, job_require, company_name, work_keyword,position_id))
        print(result)
        conn.commit()
