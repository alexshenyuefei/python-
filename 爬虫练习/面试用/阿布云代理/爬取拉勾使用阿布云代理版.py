...
URL = 'https://www.lagou.com/jobs/list_python?labelWords=&fromSearch=true&suginput='  # 北京url
...
def date_insert(item):
    try:
        with connection.cursor() as cursor:
            sql = 'INSERT INTO pythonbeijing11_30(createTime,positionName,workYear,' \
...
def first_requset():
    ...
    r = requests.post(url=URL, headers=headers, data=post_data, proxies=proxies.getproxies())
...
def next_request(page_total_number):
    for i in range(2,int(page_total_number)+1):
        ...
        post_data = {"first": "true", "pn": i, "kd": "爬虫"}
